...
For a generated Retrofit API, add `retrofit` to the application and
`retrofit_generator` plus `build_runner` as development tooling; none of those
packages is an `alphax_dio` runtime dependency.
...
Retrofit generated clients work through the ordinary Dio constructor and this
adapter:
...
Run `dart run build_runner build` in the application after adding `retrofit`
and its generator tooling. The generated API interface and DTOs remain
application-owned.

Retrofit remains the API/code-generation layer and Dio remains the application
client. Keep using normal Retrofit annotations, DTO generators, response
wrappers, `CancelToken`, `FormData`, and generated progress parameters. AlphaX
owns the injected transport and policy boundary below the adapter; it does not
replace `retrofit_generator` and does not add Retrofit as a runtime dependency.

This path was verified with maintained Retrofit/build-runner generation using
typed JSON, nullable and wrapped responses, response streaming, errors,
cancellation, redirect/options handling, multipart file upload, and generated
progress callbacks. Generator-specific Dio extensions outside that tested
surface still require their own application validation.
...
The coordinated `1.0.0-rc.5` candidate is prepared for publication. This
