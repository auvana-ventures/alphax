...
|---|---|---|
| Native transport implementation inside `alphax` | `INTENTIONALLY_UNSUPPORTED_IN_1_0` | `alphax` is pure Dart and transport-independent. `AlphaXClient` consumes an injected `AlphaXTransport`; native adapters live in `alphax_native`, and deterministic fakes live in `alphax_test`. `alphax_native.createAlphaXTransport()` is an optional automatic facade outside core. |
...
| Clear README examples | REQUIRED FOR 1.0 | `IMPLEMENTED_AND_VALIDATED` | Examples cover automatic and explicit transport selection, request, stream, files, cancellation, capabilities, protocol preference/requirement, negotiated metadata, and the default/opt-in policy path; TLS/pinning/proxy behavior is documented separately without unsafe configuration examples. The shared [`USAGE_AND_CUSTOMIZATION.md`](USAGE_AND_CUSTOMIZATION.md) guide provides the progressive setup and customization model; [`POLICIES.md`](POLICIES.md) remains the detailed policy reference. |
...
| Package documentation | REQUIRED FOR 1.0 | `IMPLEMENTED_AND_VALIDATED` | Package READMEs, the shared [`USAGE_AND_CUSTOMIZATION.md`](USAGE_AND_CUSTOMIZATION.md) user guide, detailed [`POLICIES.md`](POLICIES.md), migration notes, API inventory, architecture contract, benchmark runner guidance, and release gate exist; scoped doc/link checks pass. |
