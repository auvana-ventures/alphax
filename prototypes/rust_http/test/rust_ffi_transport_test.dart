...
void main() {
  ...
  test('bounds native streaming with credit acknowledgments', () async {
    final bounded = RustFfiClient.fromPath(
      libraryPath,
      streamChunkSize: 4,
    ...
    // Rust retains one bounded reqwest response chunk while credits are
    // exhausted in addition to the acknowledged credit window.
    expect(flow['max_buffered_bytes'], greaterThanOrEqualTo(8));
